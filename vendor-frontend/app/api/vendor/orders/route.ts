...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${VENDOR_API_URL}/v1/orders`, {
      headers: {
    ...
    console.error('Error en GET /api/vendor/orders:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { error: errorData.message || 'Error al crear orden' },
        { status: response.status }
      );
    }
    ...
    console.error('Error en POST /api/vendor/orders:', error);
    return NextResponse.json(
